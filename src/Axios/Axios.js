import axios from 'axios'


export const API_URL = 'https://raw.githubusercontent.com/ag-grid/ag-grid/master/grid-packages/ag-grid-docs/src/olympicWinnersSmall.json'
export const UNDERWORKS_URL = 'http://api.underworks.ths.agency/api/'

export const Authorisation = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..yAVpSR_KUrtIIt3P84tleF0O4ImViKyO2D4-usmNLh0'

export function axiosHandler(method, baseUrl, headers = {},data={}, params={}) {
  return axios({
    method: method,
    url: baseUrl,
    headers: headers,
    data: data,
    params: params
  })
}
